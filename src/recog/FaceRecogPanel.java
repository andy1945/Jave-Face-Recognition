...
String classiferName = "haarcascade_frontalface_alt.xml";
  ...
  // return an image
  {
    ...
    try {
        image = ImageIO.read(FaceRecogPanel.class.getResourceAsStream("/crosshairs.png"));   // read in as an image
  ...
  */
  ...
  /* write the currently recognized face name */
  {
    ...
    if (faceName != null)     // draw at bottom middle
      g2.drawString("R: " + faceName, WIDTH/2, HEIGHT-10);  
  }  // end of writeName()
     ...
     "Loading" at start time */
  {
    ...
    g2.setFont(msgFont);
    if (imageCount > 0) {
      String statsMsg = String.format("Snap: %.1f ms",
  ...
  */
  ...
  // resize, convert to grayscale, clip to FACE_WIDTH*FACE_HEIGHT, recognize
  {
    ...
    if (result == null)
      ...
      top.setRecogName(faceName, distStr,result.getMatchDistance());
    }
