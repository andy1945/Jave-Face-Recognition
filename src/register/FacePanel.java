...
public class FacePanel extends JPanel implements Runnable {
   ...
   // private static final String FACE_CASCADE_FNM = "haarcascade_frontalface_alt.xml";
    ...
    private static final String FACE_DIR = "src/trainingImages/";
    private static final String FACE_DIR_R = "/trainingImages/";
    public static String FACE_FNM = "face";
    ...
    public FacePanel() {
        ...
        System.out.println("afetr==initDetector");
    ...
    private BufferedImage loadImage(String imFnm) // return an image
    {
        ...
        try {
            ...
            image = ImageIO.read(FacePanel.class.getResourceAsStream("/crosshairs.png"));   // read in as an image
...
// CascadeClassifier faceDecetor = new CascadeClassifier(FacePanel.class.getResource("haarcascade_frontalface_alt.xml").getPath().substring(1));
...
//    }
     ...
     */ {
        ...
        if (grabber == null) {
            System.out.println("====Grabber null===");
            return;
        }
    ...
    private FrameGrabber initGrabber(int ID) {
        System.out.println("====initGrabber===");
    ...
    private void drawCrosshairs(Graphics2D g2, int xCenter, int yCenter) // draw crosshairs graphic or a red circle
    {
        if (crosshairs != null) {
  ...
  // ---------------- face saving -------------------------
    ...
    private void saveClip(BufferedImage clipIm) /* resizes to at least a standard size, converts to grayscale, 
     clips to an exact size, then saves in a standard location */ {
        ...
        saveImage(faceIm, FACE_DIR+FACE_FNM + fileCount + ".png",fileCount);
        fileCount++;
    ...
    private void saveImage(BufferedImage im, String fnm ,int num) // save image in fnm
    {
        try {
            if(num==3){
               fileCount=0; 
